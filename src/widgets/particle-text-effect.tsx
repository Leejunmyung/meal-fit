...
const DEFAULT_WORDS = ["Welcome to MealFit", "AI Diet","Personalized","Healthy",]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const baseColor = { r: 0, g: 191, b: 165 } // #00BFA5

    const variation = 30 // ±변동폭

    const newColor = {
      r: Math.min(255, Math.max(0, baseColor.r + (Math.random() * 2 - 1) * variation)),
      g: Math.min(255, Math.max(0, baseColor.g + (Math.random() * 2 - 1) * variation)),
      b: Math.min(255, Math.max(0, baseColor.b + (Math.random() * 2 - 1) * variation)),
    }
